...
/**
 * Suite de pruebas para DAOImpl.updatePersonMail()
 * Valida la actualización de correo electrónico de personas físicas.
 */
@DisplayName("Suite de pruebas de DAOImpl.updatePersonMail():")
class UpdatePersonMailTest extends DAOUpdateTest {

    @InjectMocks
    private DAOImpl daoImpl;

    private static final String ERROR_MESSAGE = "Error al actualizar el correo de la persona";

    private static final int PARAM_INDEX_EMAIL = 1;
    private static final int PARAM_INDEX_PERSON_DOCUMENT = 2;
    private static final int PARAM_INDEX_PERSON_COUNTRY = 3;
    private static final int PARAM_INDEX_PERSON_DOCUMENT_TYPE = 4;
    ...
    public String getSqlStatement() {
        return SQLStatements.UPDATE_PERSON_MAIL;
    }
    ...
    public void setStatementParameters(PreparedStatement ps, Object... params) throws SQLException {
        ...
        ps.setInt(PARAM_INDEX_PERSON_COUNTRY, UY_COUNTRY_CODE);
        ps.setInt(PARAM_INDEX_PERSON_DOCUMENT_TYPE, USER_DOCUMENT_TYPE_CI);
    }
    ...
    @DisplayName("Debe actualizar el correo de la persona correctamente con diferentes variaciones de email")
    void updatePersonMail_givenValidEmailVariations_updatesSuccessfully(
            String emailToTest,
            String displayDescription) throws Exception {

        setupSuccessfulUpdate(SINGLE_ROW_UPDATED);

        assertDoesNotThrow(() -> daoImpl.updatePersonMail(nonBusinessDatatype, emailToTest));
    ...
    void updatePersonMail_givenSQLExceptionAtDifferentPoints_throwsServiceException(
        ...
        ServiceException exception = assertThrows(ServiceException.class,
                () -> daoImpl.updatePersonMail(nonBusinessDatatype, EMAIL_VALID));
    ...
    enum SqlExceptionScenario {
        PREPARE_STATEMENT("Database connection error") {
            @Override
    ...
    void updatePersonMail_givenException_closesResourcesCorrectly() throws Exception {
    ...
    void updatePersonMail_givenNoRowsAffected_executesWithoutError() throws Exception {
        setupSuccessfulUpdate(ZERO_ROWS_UPDATED);

        assertDoesNotThrow(() -> daoImpl.updatePersonMail(nonBusinessDatatype, EMAIL_VALID));
